...
// 2010. 1. 23
// enariの改造。
// windows.hおよび要らないファイルを排除。
// using namespace std; も排除。
...
public:
	...
	// 衝突判定リストを作成する
	unsigned long GetAllCollisionList( CollisionList<T>** colList)
	{
		// リスト（配列）は必ず初期化します
		m_ColList.resetPos();
	...
	// 空間内で衝突リストを作成する
	bool GetCollisionList( unsigned long Elem, std::list<T*> &ColStac )
	{
	...
	{
		return Get2DMortonNumber( (unsigned short)((pos_x-m_fLeft)/m_fUnit_W), (unsigned short)((pos_y-m_fTop)/m_fUnit_H) );
	}
